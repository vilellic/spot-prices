...
module.exports = {
  handleQuery: function (ctx: ControllerContext) {
    ...
    const queryMode: string =
      parsed.searchParams.get("queryMode") || "LowestPrices";
    const offPeakTransferPrice = Number(
      parsed.searchParams.get("offPeakTransferPrice"),
    );
    const peakTransferPrice = Number(
      parsed.searchParams.get("peakTransferPrice"),
    );
    ...
    if (queryMode !== "AboveAveragePrices" && !numberOfHours) {
      ...
      const spotPrices = ctx.cache.get(
