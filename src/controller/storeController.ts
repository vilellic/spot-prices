...
module.exports = {
  initCacheFromDisk: function (cache: NodeCache) {
    if (!cache.has(constants.CACHED_NAME_PRICES)) {
      cache.set(
        constants.CACHED_NAME_PRICES,
        readStoredResult(constants.CACHED_NAME_PRICES),
      );
    }
    console.log("Cache contents: ");
    console.log(
      "CACHED_NAME_PRICES: " +
        JSON.stringify(cache.get(constants.CACHED_NAME_PRICES), null, 2),
    );
    const spotPrices = utils.getSpotPricesFromCache(cache);

    // Invalidate cache if current time is not in todays range
    if (
      utils.isCacheReady(cache) &&
      !utils.dateIsInPricesList(spotPrices.today, new Date())
    ) {
  ...
  updateStoredResultWhenChanged: function (
    name: string,
...
function resetStoredFiles() {
  ...
  writeToDisk(
    constants.CACHED_NAME_PRICES,
    JSON.stringify(getEmptySpotPrices()),
  );
}
